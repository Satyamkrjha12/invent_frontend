...
export default function Subscription({ data, onChange, onNext, onBack }: SubscriptionProps) {
  const activePlan = PLANS.find((p) => p.id === data.plan) || PLANS[1];
    ...
    <div className="mx-auto max-w-4xl rounded-3xl border border-orange-100 bg-white p-8 shadow-xl shadow-orange-50/50">
      <div className="mb-8 text-center">
        ...
        <div className="mt-6 flex items-center justify-center gap-3">
          <span className={`text-sm font-medium ${data.billing === "monthly" ? "text-gray-900" : "text-gray-400"}`}>
            Monthly
          </span>
          <button
            type="button"
            onClick={() => onChange("billing", data.billing === "monthly" ? "yearly" : "monthly")}
          ...
          <span className={`text-sm font-medium ${data.billing === "yearly" ? "text-gray-900" : "text-gray-400"}`}>
            Yearly <span className="rounded-full bg-orange-100 px-2.5 py-0.5 text-xs font-semibold text-orange-600">Save 20%</span>
          </span>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {PLANS.map((plan) => {
          ...
          const price = data.billing === "monthly" ? plan.priceMonthly : plan.priceYearly;
          ...
          return (
            ...
            >
              ...
              <div className="mb-5">
                <div className={`inline-flex rounded-2xl p-3 mb-4 ${isSelected ? "bg-orange-50 text-orange-600" : "bg-gray-50 text-gray-500"}`}>
                  <PlanIcon className="h-6 w-6" />
                </div>
                <h3 className="text-xl font-bold text-gray-900">{plan.name}</h3>
                <p className="mt-2 text-xs text-gray-400 leading-relaxed min-h-[32px]">{plan.description}</p>
              </div>
              ...
              <div className="mb-6">
                ...
                {data.billing === "yearly" && (
                  <p className="mt-1 text-xs text-orange-600 font-medium">Billed annually</p>
                )}
              </div>
              ...
              <ul className="space-y-3 mb-8 flex-1">
                {plan.features.map((feature, i) => (
                  <li key={i} className="flex items-start gap-2.5 text-sm">
                    <Check className={`h-4 w-4 shrink-0 mt-0.5 ${isSelected ? "text-orange-500 font-bold" : "text-gray-400"}`} />
                    <span className="text-gray-600 leading-normal">{feature}</span>
                  </li>
                ))}
              </ul>
