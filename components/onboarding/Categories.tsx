...
export default function Categories() {
  const { categories, setCategories, nextStep, backStep } = useOnboarding();
  ...
  const toggleCategory = (categoryName: string) => {
    ...
    if (isSelected) {
      updated = categories.filter((c) => c !== categoryName);
    } else {
      updated = [...categories, categoryName];
    }
  ...
  const hasSelected = categories.length > 0;
