...
export default function Sites({ data, onChange, onNext, onBack }: SitesProps) {
  ...
  const handleAddSite = (e: React.FormEvent) => {
    ...
    const updatedSites = [...data.sites, newSite];
    onChange("sites", updatedSites);
  ...
  const handleRemoveSite = (id: string) => {
    const updatedSites = data.sites.filter((site) => site.id !== id);
    onChange("sites", updatedSites);
  };
