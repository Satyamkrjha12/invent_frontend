...
import { ArrowLeft, ArrowRight, ToggleLeft, ToggleRight, Hash, QrCode, Calendar, Shield, DollarSign, User } from "lucide-react";
...
export default function AssetFields() {
  const { assetFields, setAssetFields, nextStep, backStep } = useOnboarding();

  const toggleField = (fieldId: string) => {
    const isEnabled = assetFields.includes(fieldId);
    let updated: string[];

    if (isEnabled) {
      updated = assetFields.filter((f) => f !== fieldId);
    } else {
      updated = [...assetFields, fieldId];
    }
    setAssetFields(updated);
  };
    ...
    <div className="mx-auto max-w-2xl rounded-3xl border border-orange-100 bg-white p-8 shadow-xl shadow-orange-50/50">
      ...
      <div className="space-y-4">
        {AVAILABLE_FIELDS.map((field) => {
          const isEnabled = assetFields.includes(field.id);
