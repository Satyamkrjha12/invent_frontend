...
export default function Events({ data, onChange, onSubmit, onBack }: EventsProps) {
  const toggleEvent = (eventId: string) => {
    const isEnabled = data.events.includes(eventId);
    let updated: string[];

    if (isEnabled) {
      updated = data.events.filter((e) => e !== eventId);
    } else {
      updated = [...data.events, eventId];
    }
    ...
    <div className="mx-auto max-w-2xl rounded-3xl border border-orange-100 bg-white p-8 shadow-xl shadow-orange-50/50">
      ...
      <div className="space-y-4">
        {AVAILABLE_EVENTS.map((event) => {
          const isEnabled = data.events.includes(event.id);
