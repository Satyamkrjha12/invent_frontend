...
export default function CompanyInfo() {
  const { companyInfo, updateCompanyInfo, nextStep } = useOnboarding();

  const isFormValid =
    companyInfo.companyName.trim() !== "" && companyInfo.industry !== "";
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {ORG_SIZES.map((size) => {
              const isSelected = companyInfo.orgSize === size.value;
              return (
                <button
                  key={size.value}
                  type="button"
                  onClick={() => updateCompanyInfo({ orgSize: size.value })}
